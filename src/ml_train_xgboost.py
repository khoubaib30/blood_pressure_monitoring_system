...
def plot_predictions(y_test, y_pred, output_dir="models"):
    ...
    fig.suptitle('Prédictions vs Valeurs Réelles - XGBoost', fontsize=16, fontweight='bold')
    ...
    for idx, (target_name, target_label) in enumerate(zip(target_names, target_labels)):
        ...
        ax = axes[idx]
        ax.scatter(y_true, y_pred_target, alpha=0.5, s=30, color='#2E86AB')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'xgboost_predictions_scatter.png'), 
    ...
    ax.set_title('Métriques par Target - XGBoost', fontsize=14, fontweight='bold')
    ax.set_xticks(x)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'xgboost_metrics_by_target.png'), 
                dpi=300, bbox_inches='tight')
    plt.close()
...
def main():
    ...
    joblib.dump(model, "models/xgboost_regression.pkl")
